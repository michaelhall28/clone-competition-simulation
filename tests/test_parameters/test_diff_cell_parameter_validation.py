...
def test_diff_cell_validation_missing_parameters1():
    with pytest.raises(ValidationError) as exc_info:
        p = DifferentiatedCellsValidator(tag="Full")
...
def test_diff_cell_validation2(empty_diff_cell_params, validated_time_parameters):
    ...
    assert 'Cannot run AlgorithmClass.WF algorithms with B cell' in str(exc_info)
...
@pytest.mark.parametrize(
    "r,gamma,expected_msg", [
        (0.1, None, 'Must provide both r and gamma to run with B cells'),
        (None, 0.1, 'Must provide both r and gamma to run with B cells'),
