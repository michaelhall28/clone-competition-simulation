...
The fitness of an initial clone is defined using the `initial_fitness_array`.  

If `initial_fitness_array` is defined, it must contain a fitness value for each unique initial clone.   
If no `initial_fitness_array` is provided, all initial clones will be given a fitness value of 1.  
